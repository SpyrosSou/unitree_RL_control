...
# ---------------------------------------------------------------------------
#
# 17-D observation layout:
#   [0-4]   joint_pos: shoulder_pitch | shoulder_roll | shoulder_yaw | elbow_pitch | elbow_roll
#   [5-7]   joint_vel: shoulder_pitch | shoulder_roll | shoulder_yaw
#   [8-10]  ee_pos:    x | y | z
#   [11-13] goal:      x | y | z
#   [14-16] error:     x | y | z
#
# Under left↔right (y-mirror): negate roll, yaw and every y component.
_OBS_FLIP_IDX = [1, 2, 4,     # joint_pos: shoulder_roll, shoulder_yaw, elbow_roll
                 6, 7,         # joint_vel: shoulder_roll, shoulder_yaw
                 9,            # ee_pos y
                 12,           # goal y
                 15]           # error y

# 5-D action = [shoulder_pitch, shoulder_roll, shoulder_yaw, elbow_pitch, elbow_roll]
_ACT_FLIP_IDX = [1, 2, 4]     # shoulder_roll, shoulder_yaw, elbow_roll


def _mirror_obs(obs_dict: dict) -> dict:
    """Negate the y-mirror components of a 17-D observation dict."""
    t = obs_dict["policy"].clone()
    t[:, _OBS_FLIP_IDX] *= -1.0
    return {"policy": t}


def _mirror_act(action: torch.Tensor) -> torch.Tensor:
    """Negate the y-mirror components of a 5-D action tensor."""
    a = action.clone()
    a[:, _ACT_FLIP_IDX] *= -1.0
    return a
...
def main():
    ...
    if checkpoint_path is None:
        raise ValueError(
            ...
            "or fill in arm_testing/checkpoints.yaml."
        )
