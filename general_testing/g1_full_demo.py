...
- Press T to type a new arm target at the console (same pattern as arm_testing).
...
Usage:
    ...
    # Auto-load from general_testing/checkpoints.yaml
    ~/Elm/Code/IsaacLab/isaaclab.sh -p general_testing/g1_full_demo.py
...
def _resolve_checkpoint(cli_val: str | None, yaml_keys: list[str], hardcoded: str) -> str:
    ...
    if path and not os.path.isabs(path):
        path = os.path.join(os.path.dirname(os.path.dirname(_YAML_PATH)), path)
    return path
...
class G1FullDemo:
    ...
    def _build_arm_obs(self) -> torch.Tensor:
        """Construct the 17-D (or 34-D) arm observation from the locomotion scene."""
        parts = []
        ...
        for g in arm_groups:
            ...
            joint_vel = self.robot.data.joint_vel[0, jids[:3]].unsqueeze(0)  # (1, 3)
        ...
        return torch.cat(parts, dim=-1)  # (1, 17) or (1, 34)
