...
"""Ported verbatim from ``unitree_rl_lab`` (``source/unitree_rl_lab/unitree_rl_lab/assets/
robots/unitree_actuators.py``), 2026-07-21 — see ``29dof_implementation_plan.md``.

A ``DelayedPDActuator`` subclass adding a torque-speed curve and Coulomb-style joint
friction (``Fs*tanh(vel/Va) + Fd*vel``). Confirmed (by reading ``unitree.py``'s actual
``UNITREE_G1_29DOF_CFG`` actuator definitions) that this is NOT applied to G1 by
default — G1 uses plain ``ImplicitActuatorCfg`` throughout. Ported here as available
sim2real infrastructure to adopt deliberately later (actuation delay + friction modeling
was on this project's own deferred-items list), not because the base recipe already uses
it.
"""
